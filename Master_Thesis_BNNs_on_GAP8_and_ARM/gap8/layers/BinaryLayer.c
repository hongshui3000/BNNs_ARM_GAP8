...
(
 ...
 BIN_FEATURES* __restrict__ features,
...
//#ifdef MULTICORES	
//#pragma omp parallel shared(input, output) shared(features) shared( x_0, y_0, kX2, kY2, filters_done)
//	{
//#endif
		uint32_t i, j, k;
		uint32_t ki, kj, kk;
...
#pragma omp parallel shared(input, output, features, x_0, y_0, kX2, kY2, filters_done, con)\
		 private(k,i,j,ki,kj,kk,shift,rightX,leftX,rightY,leftY,outi,outj,sum, filter, filter_index,io_index,X_diff,Y_diff, bink) num_threads(CORE_NUM_SPEC)
	{
#endif
						...
						for(ki = leftX; ki < rightX; ki++){
							for(kj = leftY; kj < rightY; kj++){
								...
								{	
									...
									sum += popcount( filter[ filter_index]^ input->memBin[ io_index]);//hardware popcount 
								}
							}
						}
						sum += ((9 - X_diff*Y_diff)*input->Z)<<4;
					}
					
					/* ReLU, Batch Normalization and Binarization using only compares */
					if(sum <= features->relu[k]) /*relu = Threshold 1 */
					{
						if(sum <= features->threshhold[k]) /*Threshold 2*/
						{ 
...
#endif
							{
								output->memBin[io_index] |= features->signGamma[k]<<shift; /*shared*/	
							}
						}
						else
						{
...
#endif
							{
								output->memBin[io_index] |= ((1-features->signGamma[k])>>1)<<shift; /*shared*/	
							}
						}
					} 
					else 
...
#endif
						{
							output->memBin[io_index] |= features->compare[k]<<shift; /*shared*/	
						}
...
#ifdef MULTICORES	
#pragma omp parallel shared(input, output, features, x_0, y_0, kX2, kY2, filters_done, con)\
		 private(k,i,j,ki,kj,kk,shift,rightX,leftX,rightY,leftY,outi,outj,sum, filter, filter_index,io_index,X_diff,Y_diff, bink)// num_threads(8)
	{
#endif
						...
						for(ki = leftX; ki < rightX; ki++){
							for(kj = leftY; kj < rightY; kj++){
								...
								{	
									filter_index = ((kX2 + ki - i)*con->kY + kY2 + kj - j)*input->Z + kk;
									io_index = (ki*input->Y + kj )*input->Z + kk; 
					//printf("filter_index: %ld, io_index: %ld \n", filter_index, io_index);
					//printf("(ki %ld *input->Y %ld + kj %ld)*inputZ %ld + kk %ld \n", ki,input->Y,kj,input->Z, kk);
