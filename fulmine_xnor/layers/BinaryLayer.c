...
)
{
    ...
    #pragma omp parallel \
    	shared(input, output) \
    	shared(features) \
    	shared( x0, y0, kX2, kY2, filters_done)
    {
    #endif
    	uint32_t i, j, k;
    	uint32_t ki, kj, kk;
		...
		for(i = x0; i < input->X - x0; i += con->dX)
		{
		    ...
		    for(j = y0; j < input->Y - y0; j += con->dY)
		    {
				...
				if((X_diff == 3) && (Y_diff == 3))
				{
					...
					case 0x3: 
						sum = 432; /* 27*16 */
						sum -= popcount( filter[  0] ^ input->memBin[ ( leftX   *input->Y + leftY 	 )*3 		]) 
				    	+ popcount( filter[  + 1]   ^ input->memBin[ ( leftX   *input->Y + leftY 	 )*3 + 1	]) 
				    	+ popcount( filter[  + 2]   ^ input->memBin[ ( leftX   *input->Y + leftY     )*3 + 2	]) 
				    	+ popcount( filter[  + 3]   ^ input->memBin[ ( leftX   *input->Y + leftY  + 1)*3 		]) 
				    	+ popcount( filter[  + 4]   ^ input->memBin[ ( leftX   *input->Y + leftY  + 1)*3 + 1	]) 
				    	+ popcount( filter[  + 5]   ^ input->memBin[ ( leftX   *input->Y + leftY  + 1)*3 + 2	]) 
				    	+ popcount( filter[  + 6]   ^ input->memBin[ ( leftX   *input->Y + leftY  + 2)*3 		]) 
				    	+ popcount( filter[  + 7]   ^ input->memBin[ ( leftX   *input->Y + leftY  + 2)*3 + 1	]) 
				    	+ popcount( filter[  + 8]   ^ input->memBin[ ( leftX   *input->Y + leftY  + 2)*3 + 2	]) 
				    	+ popcount( filter[  + 9]   ^ input->memBin[ ((leftX+1)*input->Y + leftY     )*3 		]) 
				    	+ popcount( filter[  + 10]  ^ input->memBin[ ((leftX+1)*input->Y + leftY     )*3 + 1	]) 
				    	+ popcount( filter[  + 11]  ^ input->memBin[ ((leftX+1)*input->Y + leftY     )*3 + 2	]) 
				    	+ popcount( filter[  + 12]  ^ input->memBin[ ((leftX+1)*input->Y + leftY  + 1)*3 		]) 
				    	+ popcount( filter[  + 13]  ^ input->memBin[ ((leftX+1)*input->Y + leftY  + 1)*3 + 1	]) 
				    	+ popcount( filter[  + 14]  ^ input->memBin[ ((leftX+1)*input->Y + leftY  + 1)*3 + 2	]) 
				    	+ popcount( filter[  + 15]  ^ input->memBin[ ((leftX+1)*input->Y + leftY  + 2)*3 		]) 
				    	+ popcount( filter[  + 16]  ^ input->memBin[ ((leftX+1)*input->Y + leftY  + 2)*3 + 1	]) 
				    	+ popcount( filter[  + 17]  ^ input->memBin[ ((leftX+1)*input->Y + leftY  + 2)*3 + 2	])  
				    	+ popcount( filter[  + 18]  ^ input->memBin[ ((leftX+2)*input->Y + leftY     )*3 		]) 
				    	+ popcount( filter[  + 19]  ^ input->memBin[ ((leftX+2)*input->Y + leftY     )*3 + 1	]) 
				    	+ popcount( filter[  + 20]  ^ input->memBin[ ((leftX+2)*input->Y + leftY     )*3 + 2	]) 
				    	+ popcount( filter[  + 21]  ^ input->memBin[ ((leftX+2)*input->Y + leftY  + 1)*3 		]) 
				    	+ popcount( filter[  + 22]  ^ input->memBin[ ((leftX+2)*input->Y + leftY  + 1)*3 + 1	]) 
				    	+ popcount( filter[  + 23]  ^ input->memBin[ ((leftX+2)*input->Y + leftY  + 1)*3 + 2	]) 
				    	+ popcount( filter[  + 24]  ^ input->memBin[ ((leftX+2)*input->Y + leftY  + 2)*3 		]) 
				    	+ popcount( filter[  + 25]  ^ input->memBin[ ((leftX+2)*input->Y + leftY  + 2)*3 + 1	]) 
				    	+ popcount( filter[  + 26]  ^ input->memBin[ ((leftX+2)*input->Y + leftY  + 2)*3 + 2	]);  					
				    break;
					case 0x4: 
						sum = 576; /* 36*16 */
				...
				{	
				    ...
				    sum += 16 - popcount( filter[ filter_index ] ^ input->memBin[ io_index ]); 
				}}}
				}
				sum *= 2;
