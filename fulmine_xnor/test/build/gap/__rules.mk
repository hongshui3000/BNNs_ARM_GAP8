...
test_CL_OBJS =     $(patsubst %.cpp,$(CONFIG_BUILD_DIR)/test/cl/%.o, $(patsubst %.c,$(CONFIG_BUILD_DIR)/test/cl/%.o, $(PULP_APP_CL_SRCS_test) $(PULP_CL_SRCS_test) $(PULP_APP_CL_SRCS) $(PULP_APP_SRCS) $(PULP_CL_EXTRA_SRCS_test)))
test_CL_ASM_OBJS = $(patsubst %.S,$(CONFIG_BUILD_DIR)/test/cl/%.o, $(PULP_APP_CL_ASM_SRCS_test) $(PULP_CL_ASM_SRCS_test) $(PULP_APP_CL_ASM_SRCS) $(PULP_APP_ASM_SRCS) $(PULP_CL_EXTRA_ASM_SRCS_test))
test_CL_OMP_OBJS = $(patsubst %.c,$(CONFIG_BUILD_DIR)/test/cl/omp/%.o, $(PULP_APP_CL_OMP_SRCS_test) $(PULP_CL_OMP_SRCS_test) $(PULP_APP_CL_OMP_SRCS) $(PULP_APP_OMP_SRCS) $(PULP_CL_EXTRA_OMP_SRCS_test))
...
$(CONFIG_BUILD_DIR)/test/cl/%.o: %.c
	@mkdir -p `dirname $@`
	$(PULP_CL_CC) $(test_cl_cflags) -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/test/cl/%.o: %.cpp
	@mkdir -p `dirname $@`
	$(PULP_CL_CC) $(test_cl_cflags) -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/test/cl/%.o: %.S
	@mkdir -p `dirname $@`
	$(PULP_CL_CC) $(test_cl_cflags) -DLANGUAGE_ASSEMBLY -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/test/cl/omp/%.o: %.c
	@mkdir -p `dirname $@`
	touch libgomp.spec
...
PULP_FC_EXTRA_SRCS_test = /home/msc18f9/Documents/convNet_on_PULP/api/test/build/gap/rt_conf.c /home/msc18f9/Documents/convNet_on_PULP/api/test/build/gap/rt_pad_conf.c
PULP_FC_EXTRA_ASM_SRCS_test = 
PULP_FC_EXTRA_OMP_SRCS_test = 

test_FC_OBJS =     $(patsubst %.cpp,$(CONFIG_BUILD_DIR)/test/fc/%.o, $(patsubst %.c,$(CONFIG_BUILD_DIR)/test/fc/%.o, $(PULP_APP_FC_SRCS_test) $(PULP_FC_SRCS_test) $(PULP_APP_FC_SRCS)  $(PULP_FC_EXTRA_SRCS_test)))
test_FC_ASM_OBJS = $(patsubst %.S,$(CONFIG_BUILD_DIR)/test/fc/%.o, $(PULP_APP_FC_ASM_SRCS_test) $(PULP_FC_ASM_SRCS_test) $(PULP_APP_FC_ASM_SRCS)  $(PULP_FC_EXTRA_ASM_SRCS_test))
test_FC_OMP_OBJS = $(patsubst %.c,$(CONFIG_BUILD_DIR)/test/fc/omp/%.o, $(PULP_APP_FC_OMP_SRCS_test) $(PULP_FC_OMP_SRCS_test) $(PULP_APP_FC_OMP_SRCS)  $(PULP_FC_EXTRA_OMP_SRCS_test))
...
$(CONFIG_BUILD_DIR)/test/fc/%.o: %.c
	@mkdir -p `dirname $@`
	$(PULP_FC_CC) $(test_fc_cflags) -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/test/fc/%.o: %.cpp
	@mkdir -p `dirname $@`
	$(PULP_FC_CC) $(test_fc_cflags) -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/test/fc/%.o: %.S
	@mkdir -p `dirname $@`
	$(PULP_FC_CC) $(test_fc_cflags) -DLANGUAGE_ASSEMBLY -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/test/fc/omp/%.o: %.c
	@mkdir -p `dirname $@`
	touch libgomp.spec
...
$(CONFIG_BUILD_DIR)/$(PULP_APP)/$(PULP_APP): $(test_OBJS)
	mkdir -p `dirname $@`
	$(PULP_CC) $(PULP_ARCH_LDFLAGS) -MMD -MP -o $@ $^ $(PULP_LDFLAGS_test) $(PULP_LDFLAGS)
