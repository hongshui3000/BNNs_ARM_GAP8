...
void Radix2FFT_DIF_Par(FFT_Arg_T *Arg)
{
	...
	Chunk = ((iLog2N-1)/gap8_ncore()); 
	First =  CoreId*Chunk; 
	Last = Min((First+Chunk), (iLog2N-1));
	// for (iCnt1 = 0; iCnt1 < (iLog2N-1); iCnt1++) {
	...
	iA = 0;
	/* Last Layer: W = (1, 0) */
	Chunk = ((N_fft>>1)/gap8_ncore()); 
	First =  CoreId*Chunk; 
	Last = Min(First+Chunk, (N_fft>>1));
...

{
        ...
        DmaR_Evt1 = gap8_dma_memcpy((unsigned int) Arg->Twiddles, (unsigned int) Arg->L1_Twiddles, TwidSize, DMA_COPY_IN);
        DmaR_Evt2 = gap8_dma_memcpy((unsigned int) Arg->SwapLUT,  (unsigned int) Arg->L1_SwapLUT,  LUTSize,  DMA_COPY_IN);
