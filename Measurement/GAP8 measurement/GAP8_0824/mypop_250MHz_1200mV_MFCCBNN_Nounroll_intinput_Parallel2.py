...
L1 =[]
header = 0
starttime = 1.17156984
endtime = 2.23804948
#with open('All_period_MFCC_BNNs_32000_points.csv','r') as csvfile:
with open('mypop_250MHz_1200mV_MFCCBNN_Nounroll_intinput_Parallel2.csv','r') as csvfile:
    ...
    for row in plots:
        if header == 0:
            ...
            if float(row[0]) < (starttime + 0+0.00445752) and float(row[0]) > starttime:    #line 17206 - 2077 +1       
...
alllayer_current = np.mean(alllayer)#f1_current = np.mean(f1)
...
plt.xlim(1-starttime, 2.4 -starttime)
...
#p = plt.axvspan(0, 0+0.0063759, facecolor='blue', alpha=0.6)
#p = plt.axvspan(0+0.0063759, 0+0.0256248, facecolor='green', alpha=0.6)
#p = plt.axvspan(0+0.0256248, 0+0.0690523, facecolor='gray', alpha=0.6)
