...
int main(int argc, char *argv[])
{
	...
	*/
	...
	FILE* file = fopen("model_mymodel_07_01_shift12.dat", "w");
...
static void binLayerManipulator(uint32_t filters, uint32_t LayerStartPos, uint32_t input_channels)
{
	...
	for(i = 0 ; i < filters; i++)
	{
		...
		sbData_threshhold[sbpos + i] 	= (uint32_t) th;		//Threshold2
		sbData_relu[sbpos + i] 			= (uint32_t) r;	//Threshold1
		sbData_compare[sbpos + i] 		= compare;		//signBetaStar
	}
	sbpos += filters;
}
