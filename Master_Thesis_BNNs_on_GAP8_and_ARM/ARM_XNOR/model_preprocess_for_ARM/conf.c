...
//#include "model_mydata_07_12_Norm1.dat"
...
#define FIXPOINT_SCALE	1024.0	/*< scaling factor for fixpoint */
#elif defined(FIXPOINT_32BIT)
//#define FIXPOINT_SCALE  1048576.0
//#define FIXPOINT_SCALE  65536    //2^16
//#define FIXPOINT_SCALE  4096     //2^12
//#define FIXPOINT_SCALE  1024     //2^10
#define FIXPOINT_SCALE  512     //2^9
//#define FIXPOINT_SCALE 256        //2^8
#endif
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	FILE* file = fopen("model_mymodel_16f_07_01_shift10.dat", "w");
	...
	for(i = 0; i < SL_1_END; i++)
	{
		...
		fixpoint = (int32_t) (fData[i] * FIXPOINT_SCALE);
		if (fixpoint > INT16_MAX || fixpoint < INT16_MIN)
			{
				printf("error 16 bit overflow1: %d \n", fixpoint);
				return 0;}
		...
		sprintf(str, "%hd", (int16_t) fixpoint);
		err = fputs(str, file);
		fputs(",", file);
		new_float_size++;
	}
	...
	for(i = LL_START; i < LL_END_FLOAT; i++)
	{
		...
		fixpoint = (int32_t) (fData[i] * FIXPOINT_SCALE);
		fixpoint2 = (int16_t) (fData[i] * FIXPOINT_SCALE);
		if (fixpoint != (int32_t)fixpoint2)
			{
		...
		if((checkvalue > 0 ) || (checkvalue != 0xFFFF0000))
		{
			printf("error: 16 bit overflow2: %x \n", fixpoint);
			return 0;
		}*/
		sprintf(str, "%hd", (int16_t) fixpoint);
		err = fputs(str, file);
	...
	for(i = 0; i < SL_1_END; i++)
	{
		...
		if((fixpoint > INT32_MAX ) || (fixpoint < INT32_MIN))
		{
	...
	err = fputs("}; \n", file);	
        ...
        err = fputs("const uint8_t b8Data[] = {", file);
	sbpos = 0;
        /* bin 1 */
