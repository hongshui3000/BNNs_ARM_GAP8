...
api_CL_OBJS =     $(patsubst %.cpp,$(CONFIG_BUILD_DIR)/api/cl/%.o, $(patsubst %.c,$(CONFIG_BUILD_DIR)/api/cl/%.o, $(PULP_APP_CL_SRCS_api) $(PULP_CL_SRCS_api) $(PULP_APP_CL_SRCS) $(PULP_APP_SRCS) $(PULP_CL_EXTRA_SRCS_api)))
api_CL_ASM_OBJS = $(patsubst %.S,$(CONFIG_BUILD_DIR)/api/cl/%.o, $(PULP_APP_CL_ASM_SRCS_api) $(PULP_CL_ASM_SRCS_api) $(PULP_APP_CL_ASM_SRCS) $(PULP_APP_ASM_SRCS) $(PULP_CL_EXTRA_ASM_SRCS_api))
api_CL_OMP_OBJS = $(patsubst %.c,$(CONFIG_BUILD_DIR)/api/cl/omp/%.o, $(PULP_APP_CL_OMP_SRCS_api) $(PULP_CL_OMP_SRCS_api) $(PULP_APP_CL_OMP_SRCS) $(PULP_APP_OMP_SRCS) $(PULP_CL_EXTRA_OMP_SRCS_api))
...
$(CONFIG_BUILD_DIR)/api/cl/%.o: %.c
	@mkdir -p `dirname $@`
	$(PULP_CL_CC) $(api_cl_cflags) -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/api/cl/%.o: %.cpp
	@mkdir -p `dirname $@`
	$(PULP_CL_CC) $(api_cl_cflags) -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/api/cl/%.o: %.S
	@mkdir -p `dirname $@`
	$(PULP_CL_CC) $(api_cl_cflags) -DLANGUAGE_ASSEMBLY -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/api/cl/omp/%.o: %.c
	@mkdir -p `dirname $@`
	touch libgomp.spec
...
PULP_FC_EXTRA_SRCS_api = /home/msc18f9/Documents/convNet_on_PULP/api/build/gap/rt_conf.c /home/msc18f9/Documents/convNet_on_PULP/api/build/gap/rt_pad_conf.c
PULP_FC_EXTRA_ASM_SRCS_api = 
PULP_FC_EXTRA_OMP_SRCS_api = 

api_FC_OBJS =     $(patsubst %.cpp,$(CONFIG_BUILD_DIR)/api/fc/%.o, $(patsubst %.c,$(CONFIG_BUILD_DIR)/api/fc/%.o, $(PULP_APP_FC_SRCS_api) $(PULP_FC_SRCS_api) $(PULP_APP_FC_SRCS)  $(PULP_FC_EXTRA_SRCS_api)))
api_FC_ASM_OBJS = $(patsubst %.S,$(CONFIG_BUILD_DIR)/api/fc/%.o, $(PULP_APP_FC_ASM_SRCS_api) $(PULP_FC_ASM_SRCS_api) $(PULP_APP_FC_ASM_SRCS)  $(PULP_FC_EXTRA_ASM_SRCS_api))
api_FC_OMP_OBJS = $(patsubst %.c,$(CONFIG_BUILD_DIR)/api/fc/omp/%.o, $(PULP_APP_FC_OMP_SRCS_api) $(PULP_FC_OMP_SRCS_api) $(PULP_APP_FC_OMP_SRCS)  $(PULP_FC_EXTRA_OMP_SRCS_api))
...
$(CONFIG_BUILD_DIR)/api/fc/%.o: %.c
	@mkdir -p `dirname $@`
	$(PULP_FC_CC) $(api_fc_cflags) -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/api/fc/%.o: %.cpp
	@mkdir -p `dirname $@`
	$(PULP_FC_CC) $(api_fc_cflags) -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/api/fc/%.o: %.S
	@mkdir -p `dirname $@`
	$(PULP_FC_CC) $(api_fc_cflags) -DLANGUAGE_ASSEMBLY -MMD -MP -c $< -o $@

$(CONFIG_BUILD_DIR)/api/fc/omp/%.o: %.c
	@mkdir -p `dirname $@`
	touch libgomp.spec
...
$(CONFIG_BUILD_DIR)/$(PULP_APP)/$(PULP_APP): $(api_OBJS)
	mkdir -p `dirname $@`
	$(PULP_CC) $(PULP_ARCH_LDFLAGS) -MMD -MP -o $@ $^ $(PULP_LDFLAGS_api) $(PULP_LDFLAGS)
