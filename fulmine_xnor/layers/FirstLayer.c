		    ...
		    for(j = y0; j < input->Y - y0; j += con->dY)
		    {
				...
				for(kk = 0; kk < input->Z; kk++)
				{
				    sum += ((int32_t)filter[ ((kX2 + ki - i)*con->kY + kY2 + kj - j)*input->Z + kk ] *
							(int32_t)input->memFixpoint[ (ki*input->Y + kj )*input->Z + kk ]) >> FP_SCALE_BITs;
				}}}
		...
		for(i = x0; i < input->X - x0; i += con->dX)
		{
		    ...
		    for(j = y0; j < input->Y - y0; j += con->dY)
		    {
				...
				if((rightX - leftX == 3) && (rightY - leftY == 3))
				{
					...
					case 0x2: 
						// printf("z = 2\n");
						sum += (( (int64_t)filter[  0] * (int64_t)input->memFixpoint[( leftX   *input->Y + leftY 	 )*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 1]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY 	 )*2 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 2]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 3]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*2 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 4]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 5]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*2 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 6]   * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY 	 )*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 7]   * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY 	 )*2 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 8]   * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 9]   * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*2 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 10]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 11]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*2 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 12]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY 	 )*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 13]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY 	 )*2 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 14]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 15]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*2 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 16]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*2 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 17]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*2 + 1	])>> FP_SCALE_BITs); 
					break;
					case 0x3: 
						sum += (( (int64_t)filter[  0] * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY 	 )*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 1]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY 	 )*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 2]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY      )*3 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 3]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 4]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 5]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*3 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 6]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 7]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 8]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*3 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 9]   * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY      )*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 10]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY      )*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 11]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY      )*3 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 12]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 13]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 14]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*3 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 15]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 16]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 17]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*3 + 2	])>> FP_SCALE_BITs)  
				    	+ (( (int64_t)filter[  + 18]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY      )*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 19]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY      )*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 20]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY      )*3 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 21]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 22]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 23]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*3 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 24]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*3 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 25]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*3 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 26]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*3 + 2	])>> FP_SCALE_BITs);  					
				    break;
					case 0x4: 
						sum += (( (int64_t)filter[  0] * (int64_t)input->memFixpoint[( leftX   *input->Y + leftY 	 )*4 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 1]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY 	 )*4 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 2]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY      )*4 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 3]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY      )*4 + 3	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 4]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*4   		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 5]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*4 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 6]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*4 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 7]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  1)*4 + 3	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 8]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*4  		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 9]   * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*4 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 10]  * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*4 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 11]  * (int64_t)input->memFixpoint[ ( leftX   *input->Y + leftY  +  2)*4 + 3	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 12]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY 	 )*4 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 13]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY 	 )*4 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 14]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY      )*4 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 15]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY      )*4 + 3	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 16]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*4  		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 17]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*4 + 1	])>> FP_SCALE_BITs)  
				    	+ (( (int64_t)filter[  + 18]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*4 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 19]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  1)*4 + 3	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 20]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*4  		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 21]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*4 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 22]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*4 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 23]  * (int64_t)input->memFixpoint[ ((leftX+1)*input->Y + leftY  +  2)*4 + 3	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 24]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY      )*4 		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 25]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY      )*4 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 26]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY      )*4 + 2	])>> FP_SCALE_BITs) 					
				    	+ (( (int64_t)filter[  + 27]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY      )*4 + 3	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 28]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*4  		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 29]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*4 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 30]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*4 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 31]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  1)*4 + 3	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 32]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*4  		])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 33]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*4 + 1	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 34]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*4 + 2	])>> FP_SCALE_BITs) 
				    	+ (( (int64_t)filter[  + 35]  * (int64_t)input->memFixpoint[ ((leftX+2)*input->Y + leftY  +  2)*4 + 3	])>> FP_SCALE_BITs);  				
					break;
					}
				}
