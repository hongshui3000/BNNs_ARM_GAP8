...
static void binLayerManipulator(uint32_t filters, uint32_t LayerStartPos);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	/* bin layer 1 */
	sbpos = 0;
	binLayerManipulator(BIN_1_FILTERS, BIN_1_BIAS);

	/* bin layer 2 */
	binLayerManipulator(BIN_2_FILTERS, BIN_2_BIAS);

	/* bin layer 3 */
	binLayerManipulator(BIN_3_FILTERS, BIN_3_BIAS);

	/* bin layer 4 */
	binLayerManipulator(BIN_4_FILTERS, BIN_4_BIAS);
	...
	*/
	...
	FILE* file = fopen("mypopcount_model.dat", "w");
...
static void binLayerManipulator(uint32_t filters, uint32_t LayerStartPos, uint32_t input_channels)
{
	...
	for(i = 0 ; i < filters; i++)
	{
		...
		/* relu = th1_f */
		r = bias/(2*alpha) + 16*9*input_channels/32;

		/* threshold = th2*/
		BetaStar = beta*sqrt(std+eps1) - (gamma*mean);
		th = r + BetaStar/(2*alpha*gamma);

		/* compare */
		compare = (s32_sign(BetaStar) > 0) ? 1 : 0;

		/* store in sbData */ 
		sbData_signGamma[sbpos + i] 	= signGamma;
		sbData_threshhold[sbpos + i] 	= (int32_t) th;
		sbData_relu[sbpos + i] 			= (int32_t) r;
		sbData_compare[sbpos + i] 		= compare;
	}
	sbpos += filters;
}
