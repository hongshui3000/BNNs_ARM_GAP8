...
function test()
   ...
   makeBatches(X_test, y_test)
   for i=1,nBatches do
      local inputs, labels = getBatch(i, X_test, y_test)
...
function testBatch(inputsCPU, labelsCPU)
   ...
   if opt.verbose then
      print(('Epoch: Testing [%d][%d/%d] | top1 : [%.2f (%.2f)] | top5 : [%.2f (%.2f)]'):format(
               epoch, N, nBatches, top1 , top1Sum/N, top5, top5Sum/N))
   end
end
