...
#ifdef MULTICORES	
//#pragma omp parallel shared(input, predict) shared(l) shared(invalidR, invalidL)
	{
#endif	
		uint32_t i, j, k, kk;
		int32_t sum1 = 0;
			...
			for(j = invalidL; j < input->Y - invalidR; j++)
			{
				...
				/* input z is always 4 */
				{
					for(k = 0; k < l->nFilters1; k++)
					{
						...
						inter[k] = (0 > (sum1 * l->alpha1[k])  + l->bias1[k]) ? 0: (sum1 * l->alpha1[k])  + l->bias1[k];
 ...
 )
{
	...
	//signed int sum1=0;
	//long int sum2=0;
	//signed int inter[l->nFilters1];
	float temp;
	#ifdef MULTICORES
	rt_omp_start();
	#endif
#ifdef MULTICORES	
//#pragma omp parallel shared(input, predict, l) firstprivate( invalidR, invalidL) private(i,j,k,kk,temp, inter, sum1, sum2) 
{
#endif	
	//float inter2[l->nFilters2];
	/*uint32_t i, j, k, kk;
	int32_t sum1=0;
	int64_t sum2=0;
	int32_t inter[l->nFilters1];
	float temp;*/
	//int32_t* inter = (int32_t*) rt_alloc(ALLOC_LOCATION,l->nFilters1*sizeof(int32_t));
#ifdef MULTICORES
//#pragma omp for schedule(static) 
//#pragma omp for schedule(static) //reduction(-:sum1) reduction(+:sum2)
#endif
		for(i = 0; i < input->X; i++){
			for(j = invalidL; j < input->Y - invalidR; j++)
			{
#ifdef UNFOLDED
				/* input z is always 4 ---> l->nFilters1 = 128*/
				#ifdef MULTICORES
				#pragma omp parallel for schedule(static) private(sum1) num_threads(CORE_NUM)
				#endif
				for(k = 0; k < l->nFilters1; k++)
				{
...
#pragma omp critical
{
				...
				/* Calculate the full-point layer */
				#ifdef MULTICORES
				#pragma omp parallel for schedule(static) private(kk, sum2, temp) num_threads(CORE_NUM)
				#endif
				for(k = 0; k < l->nFilters2; k++)
				{
					sum2 = 0;
					...
					for(kk = 0; kk < l->nFilters1; kk++)
					{
						sum2 += ((int64_t)l->weight2[k * l->nFilters1 + kk] * (int64_t)inter[kk]);
					}
					sum2 = sum2 >> FP_SCALE_BITs;
					temp = ((float)(sum2 + l->bias2[k])) / (float) FP_SCALE;
#ifdef MULTICORES
...
/*
				#pragma omp parallel for schedule(static) private(kk, sum2, temp)  // num_threads(8)
				for(k = 0; k < l->nFilters2/2; k++) //class 0~6
				{
					sum2 = 0;
					for(kk = 0; kk < l->nFilters1; kk++)
					{
						sum2 += ((int64_t)l->weight2_1[k * l->nFilters1 + kk] * (int64_t)inter[kk]);
					}
					sum2 = sum2 >> FP_SCALE_BITs;
					temp = ((float)(sum2 + l->bias2[k])) / (float) FP_SCALE;
#ifdef MULTICORES
#pragma omp critical
				...
				for(k = 0; k < l->nFilters2/2; k++) //class 0~6
				{
					...
					for(kk = 0; kk < l->nFilters1; kk++)
					{
						sum2 += ((int64_t)l->weight2_2[k * l->nFilters1 + kk] * (int64_t)inter[kk]);
					}
					sum2 = sum2 >> FP_SCALE_BITs;
					temp = ((float)(sum2 + l->bias2[k+7])) / (float) FP_SCALE;
#ifdef MULTICORES
#pragma omp critical
#endif
					{
				...
				for(k = 0; k < l->nFilters2/2; k++) //class 0~6
				{
					...
					for(kk = 0; kk < l->nFilters1; kk++)
					{
						sum2 += ((int64_t)l->weight2_3[k * l->nFilters1 + kk] * (int64_t)inter[kk]);
					}
					sum2 = sum2 >> FP_SCALE_BITs;
					temp = ((float)(sum2 + l->bias2[k+14])) / (float) FP_SCALE;
#ifdef MULTICORES
#pragma omp critical
#endif
					{
				...
				#pragma omp parallel for schedule(static) private(kk, sum2, temp)// num_threads(8)
				for(k = 0; k < l->nFilters2/2; k++)  //class 7~13
				{
					sum2 = 0;
					for(kk = 0; kk < l->nFilters1; kk++)
					{
						sum2 += ((int64_t)l->weight2_4[k * l->nFilters1 + kk] * (int64_t)inter[kk]);
					}
					sum2 = sum2 >> FP_SCALE_BITs;
					temp = ((float)(sum2 + l->bias2[k + 21])) / (float) FP_SCALE;
#ifdef MULTICORES
#pragma omp critical
#endif
					{
				...
				//enrolled_prediction(l, inter, predict);
...
#pragma omp critical
		...
		//rt_free(ALLOC_LOCATION, inter,l->nFilters1 * sizeof(int32_t));
#ifdef MULTICORES
	}
#endif
}
...
#pragma omp	for schedule(static) collapse(2)
		...
		for(i = 0; i < input->X; i++){
			for(j = invalidL; j < input->Y - invalidR; j++)
			{
				...
				{
					#ifdef MULTICORES
					//#pragma omp parallel for schedule(static) private(sum1)
					#endif
					for(k = 0; k < l->nFilters1; k++)
					{
...
#pragma omp critical
{
				...
				/* Calculate the full-point layer */		
				#ifdef MULTICORES
				//#pragma omp parallel for schedule(static) private(kk, sum2)
				#endif
				for(k = 0; k < l->nFilters2; k++)
				{
					sum2 = 0;
