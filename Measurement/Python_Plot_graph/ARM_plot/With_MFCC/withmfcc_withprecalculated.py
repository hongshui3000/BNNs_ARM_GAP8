...
plt.gcf().subplots_adjust(bottom=0.15,left=0.11,right=0.98)
x1 = []
y = []
totoal_cur = []
mfcc = []
...
with open('scope_30.csv','r') as csvfile:
    ...
    for row in plots:
        if header == 0:
            ...
            if float(row[0]) < starttime+0.0531 and float(row[0]) > starttime:
...
print("mfcc-time   s * 460/108 = " + str(mfcc_time) + " * " + str(460/108) + " = " + str(mfcc_time*460/108))
print("mfcc-all    I * V * s = " + str(mfcc_current) + " * " + str(3.3) + " * " + str(mfcc_time) + " * " + str(460/108) + " = " + str(mfcc_current*3.3*mfcc_time*460/108))
...
plt.xlim(-3.3-starttime, 5 -starttime)
...
p = plt.axvspan(0, 0+0.0531, facecolor='blue', alpha=0.6)
p = plt.axvspan(0+0.0531, 0+0.290902, facecolor='green', alpha=0.6)
p = plt.axvspan(0+0.290902, 0+1.65624, facecolor='gray', alpha=0.6)
p = plt.axvspan(0+1.65624, 0+1.79956, facecolor='red', alpha=0.6)
